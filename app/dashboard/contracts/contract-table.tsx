...
export default function ContractTable({ contracts }: { contracts: Contract[] }) {
        ...
        if (diffDays <= 30) return (
            <div className="flex flex-col">
                ...
                <span className="text-amber-500 dark:text-amber-400 text-xs font-medium flex items-center gap-1 mt-0.5">
                    <AlertCircle className="h-3 w-3" />Expires in {diffDays} days
                </span>
            </div>
        );
    ...
    const thClass = "text-xs font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500 h-12 px-7";
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-[#0B0F1A] text-slate-900 dark:text-slate-100 transition-colors duration-200">
            ...
            {/* TOPBAR */}
            <div className="sticky top-0 z-10 bg-white/90 dark:bg-[#0B0F1A]/95 backdrop-blur-sm border-b border-slate-200 dark:border-[#1e2d45] px-8 py-5 transition-colors duration-200">
                <Button variant="ghost" size="sm" asChild className="mb-3 -ml-2 text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-[#1a2235]">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                    <div>
                        <h1 className="text-2xl font-semibold tracking-tight text-slate-900 dark:text-white">Contract Repository</h1>
                        <p className="text-sm text-slate-400 mt-1">Manage, search, and review all your analyzed documents.</p>
                    </div>
                    <Button className="bg-blue-600 hover:bg-blue-700 text-white border-0" asChild>
                        <Link href="/dashboard/upload">
                            <Plus className="mr-2 h-4 w-4" />Upload New Contract
                        </Link>
                    </Button>
                </div>
            </div>
            ...
            <div className="p-8 space-y-6 max-w-7xl mx-auto">
                ...
                {/* TOOLBAR */}
                <div className="flex items-center justify-between gap-4 bg-white dark:bg-[#111827] px-4 py-2.5 rounded-xl border border-slate-200 dark:border-[#1e2d45] shadow-sm dark:shadow-none transition-colors duration-200">
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="outline" size="sm" className="h-9 gap-2 bg-transparent border-slate-200 dark:border-[#263652] text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:border-slate-300 dark:hover:border-blue-500/50">
                        ...
                        <DropdownMenuContent align="end" className="w-48 rounded-xl shadow-xl bg-white dark:bg-[#111827] border border-slate-200 dark:border-[#1e2d45] p-2">
                            ...
                            {(['fileUrl', 'riskScore', 'status', 'expirationDate'] as const).map((col) => (
                                ...
                                >
                                    {col === 'fileUrl' ? 'File URL' : col === 'riskScore' ? 'Risk Score' : col === 'status' ? 'Status' : 'Expiration Date'}
                                </DropdownMenuCheckboxItem>
                            ))}
                        </DropdownMenuContent>
                    </DropdownMenu>
                ...
                {/* TABLE */}
                <div className="bg-white dark:bg-[#111827] rounded-2xl border border-slate-200 dark:border-[#1e2d45] shadow-sm dark:shadow-none overflow-hidden transition-colors duration-200">
                    <Table>
                        <TableHeader className="border-b border-slate-100 dark:border-[#1e2d45]">
                            <TableRow className="hover:bg-transparent border-0">
                                ...
                                {visibleColumns.fileUrl && (
                                    <TableHead className={thClass}>File URL</TableHead>
                                )}
                            ...
                            {filteredData.length > 0 ? (
                                filteredData.map((contract) => (
                                    <TableRow key={contract.id} className="border-b border-slate-50 dark:border-[#1e2d45] hover:bg-slate-50/80 dark:hover:bg-[#1a2235]/50 transition-colors">
                                        ...
                                        {visibleColumns.fileUrl && (
                                            <TableCell className="text-slate-400 font-mono text-xs px-7 max-w-37.5 truncate">
                                                {contract.fileUrl}
                                            </TableCell>
                                        )}
                                        {visibleColumns.riskScore && (
                                            <TableCell className="px-7">{getRiskPill(contract.riskScore)}</TableCell>
                                        )}
                                        {visibleColumns.status && (
                                            <TableCell className="px-7">{getStatusBadge(contract.status)}</TableCell>
                                        )}
                                        {visibleColumns.expirationDate && (
                                    ...
                                    <TableCell colSpan={6} className="h-64">
                                        <div className="flex flex-col items-center justify-center text-center">
                                            ...
                                            <p className="text-slate-400 text-sm max-w-sm mb-5">
                                                We couldn't find any contracts matching "{searchTerm}".
                                            </p>
                                            <button
                                                onClick={() => setSearchTerm("")}
                                                className="px-4 py-2 rounded-lg border border-slate-200 dark:border-[#263652] text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:border-blue-400 dark:hover:border-blue-500/50 text-sm transition-colors"
                                            >
                                                Clear Search
                                            </button>
                                        </div>
