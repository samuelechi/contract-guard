...
export default function RecentContractsPage({ recentContracts, activeFilter, onFilterChange }: Props) {

    const filteredContracts = recentContracts.filter((contract) => {
        if (activeFilter === "High Risk") return contract.riskScore > 70;
        if (activeFilter === "Pending") return contract.status === "ANALYZING" || contract.status === "PENDING_REVIEW";
        return true;
    });
    ...
    return (
        <div>
            {/* Mobile-only filter pills (desktop ones live in the parent header) */}
            <div className="flex gap-2 mb-4 sm:hidden">
                {(["All", "High Risk", "Pending"] as FilterType[]).map((f) => (
                    <button
                        key={f}
                        onClick={() => onFilterChange(f)}
                        className={`px-3 py-1.5 rounded-lg text-xs border font-medium transition-all whitespace-nowrap ${activeFilter === f
            ...
            {filteredContracts.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-10 text-center">
                    ...
                    <p className="text-sm font-medium text-slate-600 dark:text-slate-400">
                        No {activeFilter === "All" ? "" : activeFilter.toLowerCase()} contracts
                    </p>
                    {activeFilter !== "All" && (
                        <button
